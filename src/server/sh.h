    ...
    private:
      ...
      void refresh() {
        ...
        int lastcursor = cursor,
            y = getmaxy(wBorder),
            x = getmaxx(wBorder),
            yMaxLog = y - max((int)openOrders.size(), !engine->broker.semaphore.greenButton ? 0 : 2) - 1,
            yOrders = yMaxLog;
        ...
        mvwaddstr(wBorder, 0, x-25, (string(" [   ]: ") + (!engine->broker.semaphore.greenButton ? "Start" : "Stop?") + ", [ ]: Quit!").data());
        ...
        if (!engine->broker.semaphore.greenGateway) {
          ...
          if (!engine->broker.semaphore.greenButton) {
          ...
          waddstr(wBorder, (" " + gw->quote).data());
          wattroff(wBorder, COLOR_PAIR(COLOR_GREEN));
          waddch(wBorder, !engine->broker.semaphore.greenButton ? ' ' : ':');
        }
