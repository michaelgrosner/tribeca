...
export class QuotingParametersRepository extends Repository<Models.QuotingParameters> {
  constructor(
    ...
    super("qpr",
      (p: Models.QuotingParameters) => p.buySize > 0 || p.sellSize > 0 || p.buySizePercentage > 0 || p.sellSizePercentage > 0 || p.widthPing > 0 || p.widthPong > 0 || p.widthPingPercentage > 0 || p.widthPongPercentage > 0,
